...
### Fixed
- Packaging: include `configs` as package data so `configs/default.yaml` is available after installation (fixes FileNotFoundError in `core.Config`)
...
### Fixed
- Packaging: include subpackages (`web.routes`, GUI subpackages) and ensure `web/routes/__init__.py` is present to fix import errors on some platforms
